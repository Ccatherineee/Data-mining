...
def saveResults(items, rules):
    fre_items_file = open(r'D:\python\frequent_items.txt', 'w')
    ass_rules_by_confidence = open(r'D:\python\association_rules_by_confidence.txt', 'w')
    ass_rules_by_lift = open(r'D:\python\association_rules_by_lift.txt', 'w')
    print('ok3')
    ...
    for rule, support, confidence, lift in sorted(rules, key=lambda x:x[2], reverse=True):
        pre, post = rule
    ...
    for rule, support, confidence, lift in sorted(rules, key=lambda x:x[3], reverse=True):
        pre, post = rule
...
if __name__ == "__main__":
    ...
    inFile = dataFromFile(r'C:\Users\catherine\Desktop\winemag-data_first150k (1).csv')
    minSupport = 0.01
    minConfidence = 0.3
